...
public class test1 {
    ...
    public void getCoursesTest() {
        ...
        PojoTest pj = given().urlEncodingEnabled(false).queryParam("access_token", Access_Token).relaxedHTTPSValidation().
                header("Content-Type", "application/json").expect().defaultParser(Parser.JSON).
                when().get().then().extract().response().as(PojoTest.class);

        System.out.println("Linked-in:" + pj.getLinkedIn());
        System.out.println("URL: " + pj.getUrl());


        List<WebAutomationArray> web = pj.getCourses().getWebAutomation();

        for (int i =0;i<web.size();i++)
        {

       if( pj.getCourses().getWebAutomation().get(i).getCourseTitle().equalsIgnoreCase("Selenium Webdriver Java"))
       {

           int price=pj.getCourses().getWebAutomation().get(i).getPrice();
